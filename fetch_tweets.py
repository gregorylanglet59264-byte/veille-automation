...
"""
fetch_tweets.py
Fetches tweets from a Twitter/X list using the Tweepy API v2 (works on GitHub Actions).
Falls back to opencli if running locally and Tweepy API keys are not configured.
"""
import argparse
import json
import os
...
def fetch_via_tweepy(list_id, limit, hours):
    ...
    try:
        ...
        print("Tweepy not installed, falling back to opencli", file=sys.stderr)
        return None
    ...
    if not all([api_key, api_secret, access_tok, access_sec]):
        print("Twitter API credentials not found in environment, falling back to opencli", file=sys.stderr)
        return None
    ...
    try:
        ...
        tweets.sort(key=lambda x: -x["engagement_score"])
        return tweets
...
def filter_tweets_by_age(tweets, hours):
    ...
    for t in tweets:
        if "age_hours" in t:
            # Already filtered by Tweepy start_time
            filtered.append(t)
            continue
        ...
        try:
            ...
            if age <= threshold:
                ...
                t["engagement_score"] = t.get("engagement_score", engagement)
                t["is_alert"]       = t.get("is_alert", is_alert_tweet(t.get("text", "")))
                t["is_model_trend"] = t.get("is_model_trend", is_model_trend_tweet(t.get("text", "")))
                filtered.append(t)
        except ValueError:
            continue
...
def main():
    ...
    # Try Tweepy first (GitHub Actions compatible)
    ...
    if tweets is None:
        ...
        if raw is None:
            ...
            # Write empty result so downstream scripts don't crash
